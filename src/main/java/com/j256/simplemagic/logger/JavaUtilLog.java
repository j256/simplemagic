...
public class JavaUtilLog implements Log {
	...
	@Override
	public boolean isLevelEnabled(com.j256.simplemagic.logger.Log.Level level) {
		return logger.isLoggable(levelToJavaLevel(level));
	}

	@Override
	public void log(com.j256.simplemagic.logger.Log.Level level, String msg) {
		logger.log(levelToJavaLevel(level), msg);
	}

	@Override
	public void log(com.j256.simplemagic.logger.Log.Level level, String msg, Throwable throwable) {
		logger.log(levelToJavaLevel(level), msg, throwable);
	}

	private java.util.logging.Level levelToJavaLevel(com.j256.simplemagic.logger.Log.Level level) {
		switch (level) {
			case TRACE:
